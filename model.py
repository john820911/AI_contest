...
class attenSeq2Seq(object):
    ...
    def get_batch(self, data, bucket_id):
        ...
        for _ in xrange(self.batch_size):
            ...
            encoder_pad = [utility.PAD_ID] * (encoder_size - len(encoder_input))
            encoder_inputs.append(list(reversed(encoder_input + encoder_pad)))
            decoder_pad_size = decoder_size - len(decoder_input) - 1
            decoder_inputs.append([utility.GO_ID] + decoder_input + [utility.PAD_ID] * decoder_pad_size)
        ...
        for length_idx in xrange(decoder_size):
            batch_decoder_inputs.append(
                np.array(
                    [ decoder_inputs[batch_idx][length_idx] for batch_idx in xrange(self.batch_size) ],
                    dtype=np.int32
                )
            )
            ...
            for batch_idx in xrange(self.batch_size):
                ...
                if length_idx == decoder_size - 1 or target == utility.PAD_ID:
